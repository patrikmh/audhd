...
async def test_capture_saves_raw_and_routes(session):
    with sorteraren.override(model=TestModel()):
        out = await process_capture(session, CaptureIn(raw="ring vet om provsvar"))
...
async def test_agent_failure_lands_as_raw_idea(session):
    ...
    mod.sorteraren = Boom()
    try:
        out = await process_capture(session, CaptureIn(raw="en tanke"))
    finally:
...
async def test_override_skips_agent(session):
    out = await process_capture(session, CaptureIn(raw="mjölk", override=CaptureType.shopping))
...
async def test_tags_are_linked_and_in_vocabulary(session):
    with sorteraren.override(model=TestModel()):
        await process_capture(session, CaptureIn(raw="fixa faktura till leoware"))
    links = session.exec(select(TagLink)).all()
    vocab = known_tag_vocabulary(session)
...
async def test_link_tags_idempotent(session):
    ...
    link_tags(session, ["konsult"], "task", "task-1")
    link_tags(session, ["konsult"], "task", "task-1")  # re-klassificering
    session.commit()
